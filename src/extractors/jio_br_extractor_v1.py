...
class JioBRExtractorV1(BaseExtractor):
    def __init__(self):
        super().__init__("Jio BlackRock Mutual Fund", "V1")
        self.header_keywords = ["ISIN", "NAME OF THE INSTRUMENT"]
        
    def find_header_row(self, df: pd.DataFrame) -> int:
        """
        Jio BlackRock files usually have header at row 3.
        """
    ...
    def _map_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Maps Jio BlackRock specific columns to canonical names.
        """
        new_cols = {}
            ...
            for c in range(min(5, len(row))):
                ...
                if val_str in ["GRAND TOTAL", "NET ASSETS", "TOTAL NET ASSETS", "GRAND TOTAL (AUM)"] or val_str.startswith("GRAND TOTAL"):
                    ...
                    for v in row:
                        ...
                        # Filter out percentages (1.0 or 100.0) and date stamps
    ...
    def extract(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                try:
                    # 1. Parse Scheme Name from Row 0, Col 1
                    df_meta = pd.read_excel(xls, sheet_name=sheet_name, header=None, nrows=5)
                    raw_scheme_name = None
                    
                    if len(df_meta) > 0 and len(df_meta.columns) > 1:
                        raw_scheme_name = str(df_meta.iloc[0, 1]).strip()
                    ...
                    if not raw_scheme_name or raw_scheme_name == 'nan':
                        ...
                        for r_idx in range(3):
                            for c_idx in range(min(len(df_meta.columns), 3)):
                                ...
                                if "JIOBLACKROCK" in val.upper() or ("JIO" in val.upper() and "BLACKROCK" in val.upper()):
                                    raw_scheme_name = val
                                    break
            ...
            logger.error(f"Failed to extract Jio BlackRock file: {e}")
            return []
