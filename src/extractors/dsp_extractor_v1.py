...
class DSPExtractorV1(BaseExtractor):
    def __init__(self):
        super().__init__(amc_name="DSP Mutual Fund", version="V1")
        # Standard column mapping for DSP
        # Using normalization to handle minor variations like (Rs. In lakhs)
        self.column_mapping = {
    ...
    def extract(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        with pd.ExcelFile(file_path) as xls:
            for sheet_name in xls.sheet_names:
                # Skip index or technical sheets
                ...
                # 1. Header Detection (expecting Row 3)
                ...
                global_unit = "LAKHS" # DSP standard in these files
                ...
                # 4. Scheme Info
                # DSP files have scheme name in Row 0 (merged usually)
                raw_scheme_text = str(df_raw.iloc[0, 1]).strip() if df_raw.shape[0] > 0 and df_raw.shape[1] > 1 else str(sheet_name)
                
                # Check if it contains "Portfolio as on" at Row 1, if so Row 0 is likely the name
    ...
    def _map_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            # Normalize whitespace and handle minor spelling/casing variations
