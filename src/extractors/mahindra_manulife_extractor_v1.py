...
class MahindraManulifeExtractorV1(BaseExtractor):
    def __init__(self):
        super().__init__("Mahindra Manulife Mutual Fund", "V1")
        self.header_keywords = ["ISIN", "NAME OF THE INSTRUMENT"]
        
    def find_header_row(self, df: pd.DataFrame) -> int:
        """
        Mahindra Manulife files usually have header at row 5.
        """
    ...
    def _map_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Maps Mahindra Manulife specific columns to canonical names.
        """
        new_cols = {}
    ...
    def extract(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                try:
                    ...
                    if not raw_scheme_name or raw_scheme_name == 'nan':
                        # Fallback: scan first 5 rows
                        for r_idx in range(5):
                            for c_idx in range(min(len(df_meta.columns), 3)):
                                val = str(df_meta.iloc[r_idx, c_idx]).strip()
                                if "MAHINDRA" in val.upper() and ("MANULIFE" in val.upper() or "FUND" in val.upper()):
                                    raw_scheme_name = val
                                    break
            ...
            logger.error(f"Failed to extract Mahindra Manulife file: {e}")
            return []
