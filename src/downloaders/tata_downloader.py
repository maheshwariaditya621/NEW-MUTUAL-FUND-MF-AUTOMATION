...
class TataDownloader(BaseDownloader):
    ...
    def open_session(self):
        ...
        logger.info("Persistent Chrome session opened for Tata.")
    ...
    def close_session(self):
        ...
        logger.info("Persistent Chrome session closed for Tata.")
    ...
    def download(self, year: int, month: int) -> Dict:
        ...
        if target_dir.exists():
            if (target_dir / "_SUCCESS.json").exists():
                logger.info(f"TATA: {year}-{month:02d} already complete. Skipping.")
    ...
    def _run_download_flow(self, target_year: int, target_month: int, month_name: str, download_folder: Path) -> Optional[Path]:
        ...
        try:
            ...
            filename = f"TATA_{month_name}_{target_year}.xlsx"
