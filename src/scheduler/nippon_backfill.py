# src/scheduler/nippon_backfill.py
...
def is_month_complete(year: int, month: int) -> bool:
    folder_path = Path(f"data/raw/nippon/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_nippon_backfill(
    ...
    logger.info("=" * 70)
    logger.info("NIPPON BACKFILL STARTED")
    if DRY_RUN:
    ...
    logger.info("=" * 70)
    logger.info("BACKFILL SUMMARY - NIPPON")
    logger.info("=" * 70)
    logger.info(f"Mode: {mode}")
...
if __name__ == "__main__":
    # Example usage: python -m src.scheduler.nippon_backfill --start-year 2024 --start-month 1 --end-year 2024 --end-month 12
    import argparse
    parser = argparse.ArgumentParser(description="Nippon India Mutual Fund Backfill")
    ...
    if all(v is not None for v in [args.start_year, args.start_month, args.end_year, args.end_month]):
        run_nippon_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_nippon_backfill()
