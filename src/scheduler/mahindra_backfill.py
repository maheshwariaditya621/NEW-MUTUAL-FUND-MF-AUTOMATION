# src/scheduler/mahindra_backfill.py
...
def run_backfill(start_year, start_month, end_year, end_month):
    ...
    try:
        ...
        while (current_year < end_year) or (current_year == end_year and current_month <= end_month):
            logger.info(f"\n>>> Processing Mahindra Manulife: {current_year}-{current_month:02d}")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Mahindra Manulife backfill from {args.start_year}-{args.start_month:02d} to {args.end_year}-{args.end_month:02d}")
