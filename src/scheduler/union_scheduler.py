# src/scheduler/union_scheduler.py

import time
from datetime import datetime, time as dt_time
from typing import Optional, Tuple, Dict
from src.scheduler.union_backfill import run_union_backfill
...
def run_two_stage_backfill() -> dict:
    logger.info("=" * 70)
    logger.info("UNION TWO-STAGE BACKFILL EXECUTION")
    logger.info("=" * 70)
    ...
    if has_scheduled_range_config():
        ...
        try:
            stage1_result = run_union_backfill(
    ...
    try:
        stage2_result = run_union_backfill()
    except Exception as e:
...
def run_scheduler():
    logger.info("=" * 70)
    logger.info("UNION MF SCHEDULER STARTED")
    logger.info("=" * 70)
    for schedule_time in SCHEDULE_TIMES:
                        ...
                        try:
                            ...
                            if result["total_downloaded"] > 0 or result["total_failed"] > 0:
                                notifier.notify_scheduler(
                                    ...
                                    message=f"Union completed at {now.strftime('%H:%M')}",
                                    stats={
                            ...
                            notifier.notify_scheduler(
                                ...
                                message=f"Union Error: {str(e)[:100]}",
                                stats={}
                            )
                            state.mark_executed(now, schedule_time.hour)
