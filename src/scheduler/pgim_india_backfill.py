# src/scheduler/pgim_india_backfill.py
...
from src.downloaders.pgim_india_downloader import PGIMIndiaDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    ...
    folder_path = Path(f"data/raw/pgim_india/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_pgim_india_backfill(
    ...
    logger.info("=" * 70)
    logger.info("PGIM INDIA BACKFILL STARTED")
...
if __name__ == "__main__":
    ...
    run_pgim_india_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
