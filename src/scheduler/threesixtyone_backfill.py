# src/scheduler/threesixtyone_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.threesixtyone_downloader import ThreeSixtyOneDownloader
...
def run_threesixtyone_backfill(
    ...
    logger.info("=" * 70)
    logger.info("360 ONE BACKFILL STARTED")
    logger.info("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info(f"360 ONE BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "360ONE", "downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_threesixtyone_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_threesixtyone_backfill()
