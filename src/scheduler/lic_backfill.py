...
from src.downloaders.lic_downloader import LICDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    folder_path = Path(f"data/raw/lic/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_lic_backfill(
    ...
    logger.info("=" * 70)
    logger.info("LIC BACKFILL STARTED")
    if DRY_RUN:
    ...
    logger.info("=" * 70)
    logger.info("BACKFILL SUMMARY - LIC")
    logger.info("=" * 70)
    logger.info(f"Mode: {mode}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="LIC Mutual Fund Backfill")
    ...
    if all(v is not None for v in [args.start_year, args.start_month, args.end_year, args.end_month]):
        run_lic_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_lic_backfill()
