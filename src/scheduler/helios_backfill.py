# src/scheduler/helios_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.helios_downloader import HeliosDownloader
...
def run_helios_backfill(
    ...
    logger.info("=" * 70)
    logger.info("HELIOS BACKFILL STARTED")
    ...
    logger.info("=" * 70)
    logger.info(f"HELIOS BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded Months: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "HELIOS", "months_downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_helios_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_helios_backfill()
