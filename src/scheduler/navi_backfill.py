# src/scheduler/navi_backfill.py
...
from src.downloaders.navi_downloader import NaviDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    ...
    folder_path = Path(f"data/raw/navi/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_navi_backfill(
    ...
    logger.info("=" * 70)
    logger.info("NAVI BACKFILL STARTED")
...
if __name__ == "__main__":
    ...
    run_navi_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
