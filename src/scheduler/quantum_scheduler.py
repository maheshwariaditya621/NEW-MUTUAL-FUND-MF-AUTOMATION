...
Runs Quantum backfill in TWO STAGES at fixed times:
- 07:00 AM
- 03:00 PM (15:00)
- 17:33 PM (17:33)
...
from src.scheduler.quantum_backfill import run_quantum_backfill
...
def wait_for_scheduled_time(state: SchedulerState):
    ...
    if wait_seconds > 0:
        ...
        logger.info(f"Will execute in {wait_seconds/60:.2f} minutes")
...
def run_two_stage_backfill() -> dict:
    ...
    if has_scheduled_range_config():
        ...
        try:
            stage1_result = run_quantum_backfill(
    ...
    try:
        stage2_result = run_quantum_backfill()  # No arguments = auto mode
        ...
        elif stage2_result["failed"] > 0 or stage2_result["not_published"] > 0:
                    ...
                    if state.should_run(now, scheduled_hour):
                        ...
                        try:
                            ...
                            notifier.notify_scheduler(
                                ...
                                message=f"Quantum Error: {str(e)[:100]}",
                                stats={}
                            )
