# src/scheduler/bandhan_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.bandhan_downloader import BandhanDownloader
...
def run_bandhan_backfill(
    ...
    logger.info("=" * 70)
    logger.info("BANDHAN BACKFILL STARTED")
    ...
    # downloader.open_session() # Removed persistent session
    ...
    logger.info("=" * 70)
    logger.info(f"BANDHAN BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded Months: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "BANDHAN", "months_downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_bandhan_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
