"""
Kotak Scheduler Module.

Runs Kotak backfill in TWO STAGES at fixed times:
- 07:00 AM
- 03:00 PM (15:00)
- 11:45 PM (23:45)
...
def run_two_stage_backfill() -> dict:
    ...
    if has_scheduled_range_config():
        ...
        try:
            stage1_result = run_kotak_backfill(
    ...
    try:
        stage2_result = run_kotak_backfill()  # No arguments = auto mode
...
def run_scheduler():
    ...
    """
    logger.info("=" * 70)
    logger.info("KOTAK TWO-STAGE SCHEDULER STARTED")
    logger.info("=" * 70)
                        ...
                        try:
                            ...
                            if result["total_downloaded"] > 0 or result["total_failed"] > 0:
                                notifier.notify_scheduler(
                                    ...
                                    message=f"Completed at {now.strftime('%H:%M')}",
                                    stats={
