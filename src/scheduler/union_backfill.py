# src/scheduler/union_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.union_downloader import UnionDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    ...
    # Note: Union output is data/raw/union/YYYY_MM
    folder_path = Path(f"data/raw/union/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_union_backfill(
    ...
    logger.info("=" * 70)
    logger.info("UNION BACKFILL STARTED")
    ...
    try:
        # Use persistent session for efficiency
...
if __name__ == "__main__":
    # If run directly as script, parsing args to support manual range
    import argparse
    ...
    run_union_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
