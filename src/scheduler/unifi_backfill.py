# src/scheduler/unifi_backfill.py
...
from src.downloaders.unifi_downloader import UnifiDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    ...
    folder_path = Path(f"data/raw/unifi/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_unifi_backfill(
    ...
    logger.info("=" * 70)
    logger.info("UNIFI BACKFILL STARTED")
...
if __name__ == "__main__":
    ...
    run_unifi_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
