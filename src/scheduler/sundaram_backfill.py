# src/scheduler/sundaram_backfill.py
...
from src.downloaders.sundaram_downloader import SundaramDownloader
...
def is_month_complete(year: int, month: int) -> bool:
    ...
    folder_path = Path(f"data/raw/sundaram/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_sundaram_backfill(
    ...
    logger.info("=" * 70)
    logger.info("SUNDARAM BACKFILL STARTED")
...
if __name__ == "__main__":
    ...
    run_sundaram_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
