# src/scheduler/iti_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.iti_downloader import ITIDownloader
...
def run_iti_backfill(
    ...
    logger.info("=" * 70)
    logger.info("ITI BACKFILL STARTED")
    ...
    logger.info("=" * 70)
    logger.info(f"ITI BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded Months: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "ITI", "months_downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_iti_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_iti_backfill()
