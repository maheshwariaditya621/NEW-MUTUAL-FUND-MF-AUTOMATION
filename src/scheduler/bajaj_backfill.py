# src/scheduler/bajaj_backfill.py
...
def is_month_complete(year: int, month: int) -> bool:
    folder_path = Path(f"data/raw/bajaj/{year}_{month:02d}")
    success_marker = folder_path / "_SUCCESS.json"
    return success_marker.exists()


def run_bajaj_backfill(
    ...
    logger.info("=" * 70)
    logger.info("BAJAJ BACKFILL STARTED")
    if DRY_RUN:
    ...
    if all(v is not None for v in [start_year, start_month, end_year, end_month]):
        ...
        mode = "AUTO"
        logger.info(f"Mode: {mode}")
        # Automatically check the last 3 months
    ...
    logger.info("=" * 70)
    logger.info("BACKFILL SUMMARY - BAJAJ")
    logger.info("=" * 70)
    logger.info(f"Mode: {mode}")
...
if __name__ == "__main__":
    # Example usage: python -m src.scheduler.bajaj_backfill --start-year 2025 --start-month 1 --end-year 2025 --end-month 12
    import argparse
    parser = argparse.ArgumentParser(description="Bajaj Finserv Mutual Fund Backfill")
    ...
    if all(v is not None for v in [args.start_year, args.start_month, args.end_year, args.end_month]):
        run_bajaj_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_bajaj_backfill()
