# src/scheduler/quant_backfill.py

import time
from pathlib import Path
from datetime import datetime
from typing import List, Tuple, Optional
from src.downloaders.quant_downloader import QuantDownloader
...
def run_quant_backfill(
    ...
    logger.info("=" * 70)
    logger.info("QUANT BACKFILL STARTED")
    ...
    logger.info("=" * 70)
    logger.info(f"QUANT BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded Months: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "QUANT", "months_downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_quant_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_quant_backfill()
