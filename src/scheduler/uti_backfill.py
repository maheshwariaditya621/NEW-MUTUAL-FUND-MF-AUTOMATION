...
def run_uti_backfill(
    ...
    logger.info("=" * 70)
    logger.info("UTI BACKFILL STARTED")
    ...
    logger.info("=" * 70)
    logger.info(f"UTI BACKFILL SUMMARY | Duration: {duration:.2f}s")
    logger.info(f"Downloaded Months: {downloaded} | Skipped: {skipped} | Failed: {failed} | Not Published: {not_published}")
    logger.info("=" * 70)
    
    return {"amc": "UTI", "months_downloaded": downloaded, "failed": failed, "duration": duration}
...
if __name__ == "__main__":
    ...
    if args.start_year:
        run_uti_backfill(args.start_year, args.start_month, args.end_year, args.end_month)
    else:
        run_uti_backfill()
